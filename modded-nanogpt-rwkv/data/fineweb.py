...
dataset_id = os.getenv("FW_DATASET", "HuggingFaceFW/fineweb")
remote_name = os.getenv("FW_NAME", remote_name)
local_dir = os.getenv("FW_LOCAL_DIR", local_dir)
shard_size = int(os.getenv("FW_SHARD_SIZE", args.shard_size))
streaming = os.getenv("FW_STREAMING", "0") == "1"
max_shards = int(os.getenv("FW_MAX_SHARDS", "0"))
...
fw = load_dataset(dataset_id, name=remote_name, split="train", streaming=streaming)
...
if streaming:
    iterator = map(tokenize, fw)
else:
    pool = mp.Pool(nprocs)
    iterator = pool.imap(tokenize, fw, chunksize=16)
shard_index = 0
