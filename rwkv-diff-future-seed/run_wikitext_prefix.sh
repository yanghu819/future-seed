...
env "${COMMON[@]}" MODEL=rwkv FUTURE_SEED=0 TRAIN=1 \
  ...
  python rwkv_diff_future_seed.py | tee exp/wikitext2_prefix_fs0_rwkv.log

echo "[2/3] RWKV FS=1"
env "${COMMON[@]}" MODEL=rwkv FUTURE_SEED=1 FUTURE_SEED_ALPHA_INIT=-2 TRAIN=1 \
  WEIGHTS_PATH=weights/wikitext2_prefix_fs1_rwkv.pt \
  python rwkv_diff_future_seed.py | tee exp/wikitext2_prefix_fs1_rwkv.log
...
env "${COMMON[@]}" MODEL=transformer TRAIN=1 TRANS_N_HEAD=8 \
  ...
  python rwkv_diff_future_seed.py | tee exp/wikitext2_prefix_tfmlm.log
