# DW‑JRT：在 DiffusionLLM 中用“垂直状态非因果初始化”强化全局一致性

## 一句话摘要
在 RWKV7 + 离散扩散式语言建模（DiffusionLM）中，我引入 DW‑JRT：把上一层的末状态 s_T 作为下一层的初始状态，从而在“深度维”实现对整段序列的再读与校正。最小实验显示：在跨段复制与全局约束任务上，DW‑JRT 的提升非常明显（rightcopy: 0.10 → 0.58，constr: 0.16 → 0.83），这支持“多轮优化同一序列”的机制直觉。
...
DW‑JRT 的核心就是：**把上一层的末状态 s_T 作为下一层的初始状态**。这相当于把“全序列摘要”直接提供给更深层，让它从一开始就带着全局记忆，特别适合非因果、全局一致性强的任务。

---

## 2. 方法：DW‑JRT 的实现细节
在 `rwkv-diff-dw-jrt/rwkv_diff_dw_jrt.py` 中，DW‑JRT 的做法非常克制：

- baseline：每层初始状态 s_0 = 0
- DW‑JRT：每层初始状态 s_0 = 上一层末状态 s_T
- 为避免“被 w 抹掉”，注入被写成：
  - `inject = base * sigmoid(alpha)`
  - `s = s + inject * (1 - w)`

这里 `alpha` 是每层每头一个可学习标量（默认 `DW_JRT_ALPHA_INIT=-2`，表示初始注入较弱）。
...
DW‑JRT 适合 DiffusionLM 的原因：
- 非因果：不会出现“信息泄露”问题
- 多轮修正：s_T 提供全局摘要，利于跨段约束
...
两者都是“跨字段一致性”任务，最容易检验 DW‑JRT 的有效性。
...
对比：
- baseline：`DW_JRT=0`
- DW‑JRT：`DW_JRT=1 DW_JRT_ALPHA_INIT=-2`
...
**解读：** baseline 几乎不学会规则；DW‑JRT 能明显恢复正确结构，且 PR 接近 GT。
