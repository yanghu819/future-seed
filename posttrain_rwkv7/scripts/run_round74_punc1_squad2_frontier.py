...
RECORDS = RUNS / '_round74_punc1_squad2_frontier_records.jsonl'
SUMMARY = RUNS / '_summary_round74_punc1_squad2_frontier.txt'
...
def run_one(task: Task, stage: str, cfg: str, mode: str, budget: int, steps: int, eval_every: int, val_batches: int, extra: List[str]) -> Optional[float]:
    log = RUNS / f'_round74_{task.name}_{stage}_{cfg}.log'
    cmd = [
...
def summarize() -> None:
    ...
    out = ['=' * 112, 'Round74 punc1+squad2 frontier summary', '=' * 112]
    rank = []
...
def main() -> None:
    ...
    punc_seed1 = Task(
        name='punc_seed1_frontier_recheck',
        seed=1,
    ...
    squad_seed2 = Task(
        name='squad_seed2_frontier_recheck',
        seed=2,
