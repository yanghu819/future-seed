...
# Round35: aggressive prune for real-text punc_restore transfer.
# Policy:
# - quick stage on hotpot+mbpp only
# - promote only TOP-1 FS config per task
# - require >= +0.30pp quick gain to enter med stage
...
RUN_REC=runs/_round35_punc_transfer_prune_s0_records.jsonl
SUMMARY=runs/_summary_round35_punc_transfer_prune_s0.txt
LOG=runs/_log_round35_punc_transfer_prune_s0.$(date +%Y%m%d_%H%M%S).log
: > "$RUN_REC"
...
run_train() {
  ...
  local steps="$1"; shift
  local log="runs/_round35_${task}_${stage}_${cfg}.log"
  rm -f "$log"
...
import json
from pathlib import Path
rows=[json.loads(x) for x in Path("runs/_round35_punc_transfer_prune_s0_records.jsonl").read_text().splitlines() if x.strip()]
print("="*116)
print("Round35 punc transfer (aggressive prune) summary")
print("="*116)
