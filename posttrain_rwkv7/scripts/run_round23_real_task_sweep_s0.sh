...
# Round23: real-task sweep with strict serial early-prune.
# Targets:
# - mbpp (code FIM-style long context)
# - hotpot (document QA / IE)
# - punc_restore (ASR post-processing proxy)
# - protein_ss (bio sequence labeling)
...
RUN_REC=runs/_round23_real_task_sweep_records.jsonl
SUMMARY=runs/_summary_round23_real_task_sweep_s0.txt
LOG=runs/_log_round23_real_task_sweep_s0.$(date +%Y%m%d_%H%M%S).log
: > "$RUN_REC"
...
run_train() {
  ...
  local steps="$1"; shift
  local log="runs/_round23_${task}_${stage}_${cfg}.log"
  rm -f "$log"
...
probe_bsz() {
  ...
  for b in $bsz_list; do
    local log="runs/_round23_probe_${task}_b${b}.log"
    rm -f "$log"
    set +e
...
run_task_hotpot() {
  ...
  run_with_prune "$task" "$trainer" "$PRUNE_HOTPOT" "$bsz" "${base[@]}" <<'EOF'
...
run_task_punc() {
  ...
  local bsz
  bsz=$(probe_bsz "$task" "$trainer" "10 8 6 4 3 2" "${base[@]}")
  record "{\"task\":\"$task\",\"stage\":\"probe\",\"config\":\"bsz\",\"status\":\"ok\",\"bsz\":$bsz,\"best_val_loss\":null,\"best_val_tok_acc\":null,\"run_dir\":\"\"}"

  run_with_prune "$task" "$trainer" "$PRUNE_PUNC" "$bsz" "${base[@]}" <<'EOF'
scalar_l8_trainable --alpha_lr 5e-4 --alpha_init -2 --fs_variant scalar --fs_layer_start 8 --fs_norm --fs_detach --fs_clip 1.0
scalar_l8_sched_cos --alpha_lr 0 --alpha_init -2 --fs_variant scalar --fs_layer_start 8 --fs_norm --fs_detach --fs_clip 1.0 --fs_alpha_schedule cosine --fs_alpha_min 0.4 --fs_alpha_max 1.0
head_l8 --alpha_lr 0 --alpha_init -3 --fs_variant head --alpha_head_init -3 --alpha_head_lr 5e-4 --fs_layer_start 8 --fs_norm --fs_detach --fs_clip 1.0
EOF
...
run_task_protein_ss() {
  ...
  run_with_prune "$task" "$trainer" "$PRUNE_PROTEIN" "$bsz" "${base[@]}" <<'EOF'
scalar_l10_sched_cos --alpha_lr 0 --alpha_init -2 --fs_variant scalar --fs_layer_start 10 --fs_norm --fs_detach --fs_clip 1.0 --fs_alpha_schedule cosine --fs_alpha_min 0.4 --fs_alpha_max 1.0
scalar_l10_trainable --alpha_lr 5e-4 --alpha_init -2 --fs_variant scalar --fs_layer_start 10 --fs_norm --fs_detach --fs_clip 1.0
head_l10 --alpha_lr 0 --alpha_init -3 --fs_variant head --alpha_head_init -3 --alpha_head_lr 5e-4 --fs_layer_start 10 --fs_norm --fs_detach --fs_clip 1.0
EOF
...
rows=[json.loads(x) for x in Path("runs/_round23_real_task_sweep_records.jsonl").read_text().splitlines() if x.strip()]
tasks=sorted({r["task"] for r in rows})
print("="*120)
print("Round23 real-task sweep summary (single seed, serial early-prune)")
print("="*120)
for t in tasks:
    print(f"[{t}]")
