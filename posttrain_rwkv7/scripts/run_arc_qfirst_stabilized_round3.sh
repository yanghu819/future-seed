...
# FS config: same as stabilized-r2.
ALPHA_INIT=-2
ALPHA_LR=0
FS_LAYER_START=6
FS_CLIP=1.0
...
for seed in "${SEEDS[@]}"; do
  for mode in no_fs prompt_fs; do
    echo "RUN arc question-first stabilized L=$L seed=$seed mode=$mode"
...
./.venv/bin/python summarize_arc_by_order.py \
  ...
  --fs_norm \
  --fs_detach \
  --fs_clip "$FS_CLIP" | tee runs/_summary_arc_qfirst_stabilized_r3.txt
