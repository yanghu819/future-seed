...
def parse_best(run_dir: Path) -> dict[str, float]:
    ...
    with (run_dir / "metrics.jsonl").open("r", encoding="utf-8") as f:
        for line in f:
            ...
            if "val_seq_acc" in r:
                ...
                if best_seq is None or v > best_seq:
                    best_seq = v
    ...
    return {
        ...
        "best_val_seq_acc": float(best_seq),
    }
...
def main() -> None:
    ...
    runs = []
    for cfg_path in root.glob("**/protein_contact_pair_sft/*/config.json"):
    ...
    if not runs:
        print("No matched protein_contact_pair runs yet.")
        print("filter:", flt)
        return
        ...
        by_len.setdefault(L, []).append(
            {
                ...
                "no_fs_seq": ma["best_val_seq_acc"],
                "fs_seq": mb["best_val_seq_acc"],
                "d_seq": mb["best_val_seq_acc"] - ma["best_val_seq_acc"],
            }
        )
    ...
    for L in sorted(by_len.keys()):
        ...
        d_seq = [r["d_seq"] for r in rows]
        d_loss = [r["d_loss"] for r in rows]
        n_pos = sum(1 for x in d_acc if x > 0)
        n_neg = sum(1 for x in d_acc if x < 0)
        print("=" * 110)
        print(
            f"L={L} pairs={len(rows)} mean d_acc={st.mean(d_acc):+.4f} std={st.pstdev(d_acc):.4f} | "
            f"mean d_seq={st.mean(d_seq):+.4f} std={st.pstdev(d_seq):.4f} | "
            f"mean d_loss={st.mean(d_loss):+.4f} std={st.pstdev(d_loss):.4f} | "
            f"sign(+/0/-)={n_pos}/{len(rows)-n_pos-n_neg}/{n_neg}"
        )
        for r in rows:
            print(
                "  seed={seed} d_acc={d_acc:+.4f} d_seq={d_seq:+.4f} "
                "(tok no={no_fs_acc:.4f} fs={fs_acc:.4f}; seq no={no_fs_seq:.4f} fs={fs_seq:.4f}) "
                "d_loss={d_loss:+.4f}".format(**r)
            )
