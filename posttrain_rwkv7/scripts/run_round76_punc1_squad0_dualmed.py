...
RECORDS = RUNS / '_round76_punc1_squad0_dualmed_records.jsonl'
SUMMARY = RUNS / '_summary_round76_punc1_squad0_dualmed.txt'
...
def run_one(task: Task, stage: str, cfg: str, mode: str, budget: int, steps: int, eval_every: int, val_batches: int, extra: List[str]) -> Optional[float]:
    log = RUNS / f'_round76_{task.name}_{stage}_{cfg}.log'
    cmd = [
...
def summarize() -> None:
    ...
    out = ['=' * 112, 'Round76 punc1+squad0 dualmed summary', '=' * 112]
    rank = []
...
def main() -> None:
    ...
    punc_seed1 = Task(
        name='punc_seed1_dualmed_compare',
        seed=1,
        ...
        med_budget=240, med_steps=360, med_eval_every=20, med_val_batches=3,
        prune_drop_pp=-0.5, med_gate_pp=0.0, promote_topk=2,
    )

    squad_seed0 = Task(
        name='squad_seed0_dualmed_compare',
        seed=0,
        ...
        med_budget=260, med_steps=300, med_eval_every=20, med_val_batches=3,
        prune_drop_pp=-0.5, med_gate_pp=0.1, promote_topk=2,
    )
