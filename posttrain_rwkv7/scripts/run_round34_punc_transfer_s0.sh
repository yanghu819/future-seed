...
# Round34: find transferable real-text punc_restore scenes quickly.
...
RUN_REC=runs/_round34_punc_transfer_s0_records.jsonl
SUMMARY=runs/_summary_round34_punc_transfer_s0.txt
LOG=runs/_log_round34_punc_transfer_s0.$(date +%Y%m%d_%H%M%S).log
: > "$RUN_REC"
...
run_train() {
  ...
  local steps="$1"; shift
  local log="runs/_round34_${task}_${stage}_${cfg}.log"
  rm -f "$log"
...
run_task() {
  local task="$1"; shift
  read -r _ b_acc _ < <(run_train "$task" quick baseline no_fs "$Q_BUDGET" "$Q_STEPS" "$@")
  [[ "$b_acc" == "nan" ]] && return

  read -r _ q1 _ < <(run_train "$task" quick head_l8 prompt_fs "$Q_BUDGET" "$Q_STEPS" "$@" --alpha_lr 0 --alpha_init -3 --fs_variant head --alpha_head_init -3 --alpha_head_lr 5e-4 --fs_layer_start 8 --fs_norm --fs_detach --fs_clip 1.0)
  if [[ "$q1" != "nan" ]]; then
    d1=$(python3 - <<PY
print(float("$q1")-float("$b_acc"))
PY
)
    keep1=$(python3 - <<PY
print(1 if float("$d1") >= float("$PRUNE") else 0)
PY
)
    if [[ "$keep1" == "1" ]]; then
      run_train "$task" med head_l8 prompt_fs "$M_BUDGET" "$M_STEPS" "$@" --alpha_lr 0 --alpha_init -3 --fs_variant head --alpha_head_init -3 --alpha_head_lr 5e-4 --fs_layer_start 8 --fs_norm --fs_detach --fs_clip 1.0 >/dev/null
    fi
  fi

  read -r _ q2 _ < <(run_train "$task" quick scalar_l8_sched_cos prompt_fs "$Q_BUDGET" "$Q_STEPS" "$@" --alpha_lr 0 --alpha_init -2 --fs_variant scalar --fs_layer_start 8 --fs_norm --fs_detach --fs_clip 1.0 --fs_alpha_schedule cosine --fs_alpha_min 0.4 --fs_alpha_max 1.0)
  if [[ "$q2" != "nan" ]]; then
    d2=$(python3 - <<PY
print(float("$q2")-float("$b_acc"))
PY
)
    keep2=$(python3 - <<PY
print(1 if float("$d2") >= float("$PRUNE") else 0)
PY
)
    if [[ "$keep2" == "1" ]]; then
      run_train "$task" med scalar_l8_sched_cos prompt_fs "$M_BUDGET" "$M_STEPS" "$@" --alpha_lr 0 --alpha_init -2 --fs_variant scalar --fs_layer_start 8 --fs_norm --fs_detach --fs_clip 1.0 --fs_alpha_schedule cosine --fs_alpha_min 0.4 --fs_alpha_max 1.0 >/dev/null
    fi
  fi
}

{
  echo "Round34 started: $(date)"
...
import json
from pathlib import Path
rows=[json.loads(x) for x in Path("runs/_round34_punc_transfer_s0_records.jsonl").read_text().splitlines() if x.strip()]
print("="*116)
print("Round34 punc transfer summary")
print("="*116)
