...
for seed in "${SEEDS[@]}"; do
  for mode in no_fs prompt_fs; do
    echo "RUN protein_contact_pair r2_trainable q_after L=$L seed=$seed mode=$mode"
...
./.venv/bin/python summarize_protein_contact_pair.py \
  ...
  --fs_norm \
  --fs_detach \
  --fs_clip "$FS_CLIP" | tee runs/_summary_protein_contact_pair_qafter_len2048_r2_trainable_s012.txt
