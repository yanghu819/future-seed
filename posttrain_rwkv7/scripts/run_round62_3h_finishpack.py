...
RECORDS = RUNS / '_round62_3h_finishpack_records.jsonl'
SUMMARY = RUNS / '_summary_round62_3h_finishpack.txt'
...
def run_one(task: Task, stage: str, cfg: str, mode: str, budget: int, steps: int, eval_every: int, val_batches: int, extra: List[str]) -> Optional[float]:
    log = RUNS / f'_round62_{task.name}_{stage}_{cfg}.log'
    cmd = [
...
def run_task(task: Task) -> None:
    ...
    for c in task.cands:
        ...
        if dpp < task.prune_drop_pp:
            append({
                ...
                'config': c.name,
                'status': 'pruned',
                'reason': f'quick_drop {dpp:+.2f}pp < {task.prune_drop_pp:+.2f}pp',
            })
    ...
    if best_dpp < task.med_gate_pp:
        append({
            ...
            'config': best[0].name,
            'status': 'pruned',
            'reason': f'best_quick {best_dpp:+.2f}pp < med_gate {task.med_gate_pp:+.2f}pp',
        })
        return
...
def summarize() -> None:
    ...
    out = ['=' * 112, 'Round62 3h finishpack summary', '=' * 112]
    ...
    for task in sorted({r['task'] for r in rows if 'task' in r}):
        ...
        med_b = [r for r in rows if r.get('task') == task and r.get('stage') == 'med' and r.get('config') == 'baseline' and r.get('status') == 'ok']
        med_f = [r for r in rows if r.get('task') == task and r.get('stage') == 'med' and r.get('config') != 'baseline' and r.get('status') == 'ok']
        if med_b and med_f:
            bmed = float(med_b[0]['best_val_tok_acc'])
            best_med = max(med_f, key=lambda x: float(x['best_val_tok_acc']))
            ...
            score_rows.append((task, 'med', best_med['config'], dmed))
        else:
            ...
            if qfs:
                ...
                score_rows.append((task, 'quick', best_q['config'], dq))

        prunes = [r for r in rows if r.get('task') == task and r.get('status') == 'pruned']
        if prunes:
            out.append('  pruned:')
            for p in prunes:
    ...
    for t, stg, cfg, d in sorted(score_rows, key=lambda x: x[3], reverse=True):
        out.append(f'  {t:28s} stage={stg:5s} cfg={cfg:24s} d_acc={d:+.2f}pp')
...
def main() -> None:
    ...
    mbpp_seed2 = Task(
        name='mbpp_strict_seed2_confirm',
        seed=2,
        base_args=['--ds','mbpp','--ds_cfg','','--train_split','train','--val_split','test','--n_train','340','--n_val','100','--min_chars','24','--max_chars','360','--fill_notes_to_max','--note_pool_size','512','--max_prompt_tokens','1536','--min_prompt_tokens','384','--max_answer_tokens','160','--bsz','12'],
        cands=[
            ...
            Candidate('head_l10_midlr',['--alpha_lr','0','--alpha_init','-2','--fs_variant','head','--alpha_head_init','-2','--alpha_head_lr','7e-4','--fs_layer_start','10','--fs_norm','--fs_detach','--fs_clip','1.0']),
        ],
        ...
        prune_drop_pp=-0.5, med_gate_pp=0.2, run_med=True,
    )

    squad_rescue = Task(
        name='squad_strict_seed1_rescue',
        seed=1,
        base_args=['--ds','squad','--ds_cfg','','--train_split','train','--val_split','validation','--n_train','1200','--n_val','160','--min_chars','64','--max_chars','260','--fill_notes_to_max','--note_pool_size','768','--max_prompt_tokens','1408','--min_prompt_tokens','448','--max_answer_tokens','128','--bsz','12'],
        cands=[
            Candidate('scalar_l8_sched_ultra',['--alpha_lr','0','--alpha_init','-2','--fs_variant','scalar','--fs_layer_start','8','--fs_norm','--fs_detach','--fs_clip','1.0','--fs_alpha_schedule','cosine','--fs_alpha_min','0.8','--fs_alpha_max','1.0']),
            Candidate('scalar_l10_sched_gentle',['--alpha_lr','0','--alpha_init','-2','--fs_variant','scalar','--fs_layer_start','10','--fs_norm','--fs_detach','--fs_clip','1.0','--fs_alpha_schedule','cosine','--fs_alpha_min','0.7','--fs_alpha_max','1.0']),
            Candidate('scalar_l8_train5e5',['--alpha_lr','5e-5','--alpha_init','-2','--fs_variant','scalar','--fs_layer_start','8','--fs_norm','--fs_detach','--fs_clip','1.0']),
        ],
        quick_budget=160, quick_steps=160, quick_eval_every=20, quick_val_batches=2,
        med_budget=300, med_steps=320, med_eval_every=20, med_val_batches=3,
        prune_drop_pp=-0.5, med_gate_pp=0.2, run_med=True,
    ...
    punc_scout = Task(
        ...
        med_budget=280, med_steps=420, med_eval_every=20, med_val_batches=3,
        prune_drop_pp=-0.5, med_gate_pp=0.2, run_med=True,
    )

    # Priority first: lock down MBPP usefulness signal, then rescue SQuAD, then scout PUNC.
    run_task(mbpp_seed2)
    run_task(squad_rescue)
    run_task(punc_scout)
    summarize()
