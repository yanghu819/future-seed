...
# FS config: stabilized recipe, but weaker fixed seed injection.
ALPHA_INIT=-4
ALPHA_LR=0
FS_LAYER_START=6
FS_CLIP=1.0
...
for seed in "${SEEDS[@]}"; do
  for mode in no_fs prompt_fs; do
    echo "RUN hotpot q_after stabilized alpha=-4 L=$L seed=$seed mode=$mode"
...
./.venv/bin/python summarize_hotpot_by_len.py \
  ...
  --fs_norm \
  --fs_detach \
  --fs_clip "$FS_CLIP" | tee runs/_summary_hotpot_qafter_stabilized_len4096_r8_alpha_m4_s012.txt
