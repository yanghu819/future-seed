...
for seed in "${SEEDS[@]}"; do
  for mode in no_fs prompt_fs; do
    echo "RUN hotpot q-after r12 L=$L lstart=$FS_LAYER_START alpha=$ALPHA_INIT seed=$seed mode=$mode"
...
./.venv/bin/python summarize_hotpot_by_len.py \
  ...
  --fs_norm \
  --fs_detach \
  --fs_clip 1.0 | tee runs/_summary_hotpot_qafter_stabilized_len4096_r12_lstart10_alpha-3_s01234.txt
