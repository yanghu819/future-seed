...
"""Protein contact-pair QA SFT probe for Future-Seed.

Real task (tiny AlphaFold-style signal):
  Given a protein sequence and queried residue index pairs, predict contact bits.
"""
...
def _load_rows(ds: str, split: str) -> list[tuple[str, str, list[list[float]], list[bool]]]:
    obj = load_dataset(ds, split=split)
    out: list[tuple[str, str, list[list[float]], list[bool]]] = []
    for ex in obj:
        name = _clean(ex.get("name", ""))
        seq = _safe_seq(_clean(ex.get("seq", "")))
        tertiary = ex.get("tertiary")
        valid_mask = ex.get("valid_mask")
        if not seq or tertiary is None or valid_mask is None:
            continue
        if not isinstance(tertiary, list) or not isinstance(valid_mask, list):
            continue
        if len(seq) != len(tertiary) or len(seq) != len(valid_mask):
            continue
        ok = True
        t2: list[list[float]] = []
        for v in tertiary:
            if not isinstance(v, (list, tuple)) or len(v) != 3:
                ok = False
                break
            t2.append([float(v[0]), float(v[1]), float(v[2])])
        if not ok:
            continue
        vm = [bool(x) for x in valid_mask]
        out.append((name, seq, t2, vm))
    return out
...
def _crop_row(
    ...
    max_seq_len: int,
    rng: random.Random,
) -> tuple[str, list[list[float]], list[bool]]:
    L = len(seq)
    if L <= max_seq_len:
        return seq, tertiary, valid_mask
    start = rng.randrange(0, L - max_seq_len + 1)
    end = start + max_seq_len
    return seq[start:end], tertiary[start:end], valid_mask[start:end]


def _build_note_pool(
    rows: list[tuple[str, str, list[list[float]], list[bool]]],
    *,
    ...
    out: list[list[int]] = []
    for i in idxs:
        name, seq, _t, _m = rows[int(i)]
        s = seq[:max_note_seq_len]
        txt = f"Protein note {name}: {s}"
        ids = tok.encode(txt)
        if ids:
            out.append(ids)
...
def _sample_pairs(
    ...
    idxs = [i for i, v in enumerate(valid_mask) if v]
    if len(idxs) < 2:
        return [], ""
    pair_set = set()
    ...
    if pair_sampling == "balanced":
        n_pos_target = max(1, min(num_pairs - 1, int(round(num_pairs * pos_ratio))))
        n_neg_target = num_pairs - n_pos_target
        pos_items: list[tuple[tuple[int, int], str]] = []
        neg_items: list[tuple[tuple[int, int], str]] = []
        max_try = max(800, num_pairs * 300)
        t = 0
        while (len(pos_items) < n_pos_target or len(neg_items) < n_neg_target) and t < max_try:
            t += 1
            ...
            if (a, b) in pair_set:
                continue
            pair_set.add((a, b))
            d = _dist(tertiary[a], tertiary[b])
            bit = "1" if d <= cutoff else "0"
            if bit == "1":
                pos_items.append(((a, b), bit))
            else:
                neg_items.append(((a, b), bit))

        if len(pos_items) < n_pos_target or len(neg_items) < n_neg_target:
            return [], ""

        chosen = pos_items[:n_pos_target] + neg_items[:n_neg_target]
        rng.shuffle(chosen)
        for (a, b), bit in chosen:
            pairs.append((a, b))
            bits.append(bit)
        return pairs, "".join(bits)

    # random sampling (default)
    max_try = max(200, num_pairs * 40)
    t = 0
    while len(pairs) < num_pairs and t < max_try:
        t += 1
        i = idxs[rng.randrange(len(idxs))]
        j = idxs[rng.randrange(len(idxs))]
        if i == j:
        ...
        d = _dist(tertiary[a], tertiary[b])
        pairs.append((a, b))
        bits.append("1" if d <= cutoff else "0")
...
def _build_prompt_ids(
    *,
    tok: RWKVWorldTokenizer,
    seq: str,
    pairs_1based: list[tuple[int, int]],
    ...
    q_txt = ",".join(f"{i}-{j}" for (i, j) in pairs_1based)
    inst = (
        "Task: Protein contact QA.\n"
        "For each queried residue pair, output 1 if in contact, else 0.\n"
        "Output bits in the same order, no spaces.\n"
    )
...
def _build_examples(
    *,
    ...
    if not rows:
        raise RuntimeError("No rows for split.")

    note_pool_ids = _build_note_pool(
        rows, tok=tok, rng=rng, pool_size=note_pool_size, max_note_seq_len=max_note_seq_len
    ...
    for i in idxs:
        _name, seq0, ter0, vm0 = rows[int(i)]
        seq, ter, vm = _crop_row(seq0, ter0, vm0, max_seq_len=max_seq_len, rng=rng)
        if len(seq) < min_seq_len:
            continue
        pairs, bits = _sample_pairs(
            ter,
            vm,
            num_pairs=num_pairs,
            min_sep=min_sep,
        ...
        if not pairs or not bits:
            continue
        q_pairs_1b = [(a + 1, b + 1) for (a, b) in pairs]
        p_ids = _build_prompt_ids(
            tok=tok,
            seq=seq,
            pairs_1based=q_pairs_1b,
...
def main() -> None:
    ...
    ap.add_argument("--ds", type=str, default="heya5/protein_contact_map")
    ap.add_argument("--train_split", type=str, default="train")
    ap.add_argument("--val_split", type=str, default="valid")
    ...
    ap.add_argument("--min_sep", type=int, default=8)
    ap.add_argument("--contact_cutoff", type=float, default=8.0)
    ap.add_argument("--pair_sampling", choices=["random", "balanced"], default="random")
    ...
    run_root = Path(args.run_dir) / time.strftime("%Y%m%d-%H%M%S") / "protein_contact_pair_sft" / args.mode
    ...
    cache_path = Path(args.cache_dir) / f"protein_contact_pair_tok_{key}.pt"
    ...
    if cache_path.exists():
        ...
        print("Loading protein contact dataset...")
        train_rows = _load_rows(args.ds, args.train_split)
        val_rows = _load_rows(args.ds, args.val_split)
