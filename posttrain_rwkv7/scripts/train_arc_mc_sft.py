...
"""ARC multiple-choice SFT probe for Future-Seed post-training (real task).

Goal: show Future-Seed helps post-training when prompt order is causal-unfriendly.
...
Controls:
  - options-first (default): options appear before the question (hard for causal models)
  - question-first (--q_first): question appears before options (causal-friendly)

Metric: teacher-forced (loss + token acc) on the answer span.
For ARC, we supervise the option label (e.g. "A").
"""
...
def _format_choices(choices: list[dict]) -> Tuple[str, List[str]]:
    rows = []
    labels = []
    for c in choices:
        lab = _clean(c.get("label", ""))
    ...
    # Keep a stable order to reduce variance.
...
def build_examples(
    *,
    ...
    for i in idxs:
        ex = ds_obj[int(i)]
        # ai2_arc schema differs across versions:
        # - question: str, choices: {label: [...], text: [...]}
        # - question: {stem: str, choices: [...]}
        q_raw = ex.get("question", "")
        if isinstance(q_raw, dict):
            q = _clean(q_raw.get("stem", ""))
            choices_raw = q_raw.get("choices", [])
        else:
            q = _clean(q_raw)
        ...
        if isinstance(choices_raw, dict):
            labels = choices_raw.get("label", []) or choices_raw.get("labels", [])
            texts = choices_raw.get("text", []) or choices_raw.get("texts", [])
            ...
            elif choices_raw and isinstance(choices_raw[0], (list, tuple)) and len(choices_raw[0]) >= 2:
                choices = [{"label": c[0], "text": c[1]} for c in choices_raw]  # type: ignore[index]

        if not isinstance(choices, list) or len(choices) < 2:
            continue

        choices_txt, labels = _format_choices(choices)
        if not choices_txt or not labels:
            continue

        a = _clean(ex.get("answerKey", ""))
        if not a:
            continue
        a = a.strip()
        if len(a) > 1:
        ...
        # Minimal instruction; keep format fixed across samples.
        if q_first:
            prompt = f"Question:\n{q}\n\nOptions:\n{choices_txt}\n\nAnswer: "
        else:
            prompt = f"Options:\n{choices_txt}\n\nQuestion:\n{q}\n\nAnswer: "
        ...
        if len(p_ids) > max_prompt_tokens:
            # Keep the suffix (Question + Answer:) intact by trimming from the left.
    ...
    if len(out) < n:
        raise RuntimeError(f"Only built {len(out)} examples (wanted {n}). Try lowering --n_train/--n_val.")
    return out
...
def main() -> None:
    ...
    ap.add_argument("--q_first", action="store_true", help="Question before options (causal-friendly control).")
    ...
    cache_path = cache_dir / f"arc_mc_tok_{cache_key}.pt"
    ...
    run_root = Path(args.run_dir) / time.strftime("%Y%m%d-%H%M%S") / "arc_mc_sft" / args.mode
    ...
    fs_in_b = None
    if args.fs_in_gate:
        # Start from zeros so the initial behavior matches the scalar alpha gate.
    ...
    while True:
        ...
        # Remaining answer tokens (rare for option labels, but keep generic)
