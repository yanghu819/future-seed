...
# Round37: fix dataset availability for WikiText/AGNews and quickly re-check FS utility.
...
RUN_REC=runs/_round37_punc_wiki_agnews_offlinefix_s0_records.jsonl
SUMMARY=runs/_summary_round37_punc_wiki_agnews_offlinefix_s0.txt
LOG=runs/_log_round37_punc_wiki_agnews_offlinefix_s0.$(date +%Y%m%d_%H%M%S).log
: > "$RUN_REC"
...
run_train() {
  ...
  local steps="$1"; shift
  local log="runs/_round37_${task}_${stage}_${cfg}.log"
  rm -f "$log"
...
run_task_top1() {
  ...
  read -r _ q_head _ < <(run_train "$task" quick head_l8 prompt_fs "$Q_BUDGET" "$Q_STEPS" "${common_args[@]}" --alpha_lr 0 --alpha_init -3 --fs_variant head --alpha_head_init -3 --alpha_head_lr 5e-4 --fs_layer_start 8 --fs_norm --fs_detach --fs_clip 1.0)
  read -r _ q_scalar _ < <(run_train "$task" quick scalar_l8_sched_cos prompt_fs "$Q_BUDGET" "$Q_STEPS" "${common_args[@]}" --alpha_lr 0 --alpha_init -2 --fs_variant scalar --fs_layer_start 8 --fs_norm --fs_detach --fs_clip 1.0 --fs_alpha_schedule cosine --fs_alpha_min 0.4 --fs_alpha_max 1.0)

  [[ "$q_head" == "nan" || "$q_scalar" == "nan" ]] && return

  local d_head d_scalar best_cfg best_delta
  d_head=$(python3 - <<PY
...
PY
    best_cfg="head_l8"; best_delta="$d_head"
  else
    best_cfg="scalar_l8_sched_cos"; best_delta="$d_scalar"
  fi
...
PY
  ...
  if [[ "$keep" == "1" ]]; then
    if [[ "$best_cfg" == "head_l8" ]]; then
      run_train "$task" med head_l8 prompt_fs "$M_BUDGET" "$M_STEPS" "${common_args[@]}" --alpha_lr 0 --alpha_init -3 --fs_variant head --alpha_head_init -3 --alpha_head_lr 5e-4 --fs_layer_start 8 --fs_norm --fs_detach --fs_clip 1.0 >/dev/null
    else
      run_train "$task" med scalar_l8_sched_cos prompt_fs "$M_BUDGET" "$M_STEPS" "${common_args[@]}" --alpha_lr 0 --alpha_init -2 --fs_variant scalar --fs_layer_start 8 --fs_norm --fs_detach --fs_clip 1.0 --fs_alpha_schedule cosine --fs_alpha_min 0.4 --fs_alpha_max 1.0 >/dev/null
    fi
  fi
}

{
  echo "Round37 started: $(date)"

  # Actively pre-download to avoid transient httpx connect failures.
  ./.venv/bin/python - <<'PY'
from datasets import load_dataset
print("prefetch wikitext...")
load_dataset("wikitext", "wikitext-2-raw-v1", split="train")
print("prefetch ag_news...")
load_dataset("ag_news", split="train")
print("prefetch done")
PY

  run_task_top1 punc_wikitext \
...
import json
from pathlib import Path
rows=[json.loads(x) for x in Path("runs/_round37_punc_wiki_agnews_offlinefix_s0_records.jsonl").read_text().splitlines() if x.strip()]
print("="*116)
print("Round37 wiki/agnews offline-fix summary")
print("="*116)
