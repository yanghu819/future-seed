...
# Give a bit more wall-clock since L=4096 is slower.
TIME_BUDGET=480
N_TRAIN=1000
N_VAL=200
MIN_PROMPT=1536
...
for seed in "${SEEDS[@]}"; do
  for mode in no_fs prompt_fs; do
    echo "RUN hotpot q_after stabilized L=$L seed=$seed mode=$mode"
...
./.venv/bin/python summarize_hotpot_by_len.py \
  ...
  --fs_norm \
  --fs_detach \
  --fs_clip "$FS_CLIP" | tee runs/_summary_hotpot_qafter_stabilized_len4096_r6_s0.txt
