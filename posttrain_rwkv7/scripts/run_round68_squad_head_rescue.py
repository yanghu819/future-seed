...
RECORDS = RUNS / '_round68_squad_head_rescue_records.jsonl'
SUMMARY = RUNS / '_summary_round68_squad_head_rescue.txt'
...
def run_one(task: Task, stage: str, cfg: str, mode: str, budget: int, steps: int, eval_every: int, val_batches: int, extra: List[str]) -> Optional[float]:
    log = RUNS / f'_round68_{task.name}_{stage}_{cfg}.log'
    cmd = [
...
def summarize() -> None:
    ...
    out = ['=' * 112, 'Round68 squad head rescue summary', '=' * 112]
    rank = []
...
def main() -> None:
    ...
    base = ['--ds','squad','--ds_cfg','','--train_split','train','--val_split','validation','--n_train','1200','--n_val','160','--min_chars','64','--max_chars','260','--fill_notes_to_max','--note_pool_size','1024','--max_prompt_tokens','1536','--min_prompt_tokens','512','--max_answer_tokens','128','--bsz','12']
