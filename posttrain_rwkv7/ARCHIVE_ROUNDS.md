# Archived Round Chronology

This file preserves the historical per-round notes that were previously appended to `posttrain_rwkv7/README.md`. It is kept for provenance and paper traceability, not as the primary entrypoint for new readers.

For the current project state, start from `README.md`. For raw operator logs, see `results/_rolling_round_log.md` and `paper/DETAILED_EXPERIMENT_LOG.md`.
