...
| Metrics consistency between paper tables and shipped snapshot boundary | `verify_metrics_snapshot.py`, `data/metrics.json`, shipped `runs/_summary_*.txt`, shipped `runs/_round*_records.jsonl` | `python3 paper/neurips2025/verify_metrics_snapshot.py` | **Fully reproducible** |
...
| Synthetic evidence as rendered in the paper | `rwkv-diff-future-seed/` plus paper-side `data/metrics.json` summary | inspect code/logs and rebuild the paper | **Auditable from snapshot**; full rerun depends on local environment |
| Curated post-training main table | `data/metrics.json`, `TASK_MATRIX.md`, shipped queue files, shipped round summaries and records | inspect the shipped records and rebuild the paper | **Auditable from snapshot** |
...
- If the goal is to rebuild the paper and audit the reported numbers against the shipped archive boundary, the snapshot is sufficient.
...
- This is why the paper keeps a conservative distinction between `fully reproducible`, `auditable from snapshot`, and `not fully reproducible`.
