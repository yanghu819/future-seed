...
All experiments use an RWKV7 backbone with prompt-time Future-Seed only: generation remains left-to-right and causal at decode time. The post-training scripts default to the checkpoint \texttt{assets/weights/rwkv7-g1d-0.1b-20260129-ctx8192.pth}. The real-task campaign uses a single NVIDIA RTX 4090 and serial quick-to-medium screening to conserve wall-clock budget.
...
\begin{itemize}
    ...
    \item the synced summary files for rounds 783--808 under \texttt{posttrain\_rwkv7/runs/};
    \item the synced JSONL records for rounds 783--808 under \texttt{posttrain\_rwkv7/runs/};
    \item this paper package under \texttt{paper/neurips2025/}.
\end{itemize}
A compact build path is provided in the paper package via \texttt{build.sh}. The paper tables are regenerated deterministically from committed metrics in \texttt{data/metrics.json} using \texttt{render\_tables.py}. The build does not scrape external services.
