...
We make three contributions. First, we present a compact mechanism view of Future-Seed as cross-layer state feedback for in-place constraint repair. Second, we show strong synthetic evidence on future-aware sanity tasks, permutation repair, and Sudoku. Third, we report a large single-GPU post-training audit with both positive and negative findings: \texttt{protein\_ss} is the strongest repeatable real-task family, while \texttt{hotpot}, \texttt{punc}, and \texttt{squad} show smaller positive pockets and \texttt{mbpp\_longctx}/\texttt{arc\_mc} remain mixed.
...
All experiments use an RWKV7 backbone with prompt-time Future-Seed only. The post-training campaign is run on a single NVIDIA RTX 4090, using serial quick-to-medium screening to keep wall-clock budget bounded.

\paragraph{Synthetic tasks.}
We evaluate RIGHTCOPY/CONSTR as minimal future-aware probes, KVSORT as in-place permutation repair, PERMFILL as anchored permutation filling with length extrapolation, and 4$\times$4 Sudoku as structured constraint repair.

\paragraph{Real-task audit.}
The real-task campaign covers protein secondary-structure labeling, HotpotQA-style multi-hop QA, long-context MBPP completion, ARC multiple choice, punctuation restoration, and a set of lower-ROI constraint proxies. The final fastdiscover queues use a two-stage policy: prune candidates below $-0.5$pp at quick stage, and only promote top candidates above $+0.8$pp to medium budget.

\paragraph{Evaluation summary.}
For synthetic tasks we report direct task metrics such as token accuracy, exact match, and solve-rate. For the post-training audit we aggregate medium-stage Future-Seed vs. baseline deltas in percentage points (pp) by task family, together with positive-count summaries and qualitative judgments.
...
Table~\ref{tab:posttrain-main} summarizes the full post-training archive by task family. Two patterns matter.

First, the mechanism is \emph{not} a universal gain switch. The strongest repeatable family is \texttt{protein\_ss}, which shows both a strong best case and a positive median over a large number of medium-stage comparisons. \texttt{hotpot} is weaker in absolute magnitude but still consistently positive enough to remain a credible supporting family. \texttt{punc} and \texttt{squad} also show small positive pockets.

Second, the most exciting spikes are not the most trustworthy claims. \texttt{mbpp\_longctx} and \texttt{arc\_mc} both produce large wins in parts of the search, but neither survives a strict held-out confirmation pass. In other words, Future-Seed can create real-task upside, but the strongest real-task claims are still the ones that survive repeated queue pressure rather than the largest one-off gains.

We therefore frame the real-task evidence conservatively. The paper can defend \texttt{protein\_ss} as the strongest repeatable family under the current recipe, and \texttt{hotpot}/\texttt{punc}/\texttt{squad} as modest supporting signals. It cannot honestly claim stable held-out confirmation on \texttt{mbpp\_longctx} or \texttt{arc\_mc} yet.

\section{Limitations and Negative Results}
This work has four important limitations. First, the post-training campaign is compute-constrained: all decisions are made under a single-GPU search budget, so the resulting archive reflects a pragmatic search policy rather than a clean, balanced hyperparameter study. Second, the aggregated family statistics in Table~\ref{tab:posttrain-main} are not independent repeated trials; they summarize a sequential exploration process. Third, the positive real-task evidence is uneven across families, and the strict confirmation queues fail for the most ambitious real-task headlines. Fourth, the synthetic tasks are intentionally sharp and mechanistic, which helps isolate the effect of Future-Seed but also narrows the external validity of the claim.
...
Future-Seed sits between three lines of work: recurrent or state-space sequence models such as RWKV~\citep{peng2023rwkv} and Mamba~\citep{gu2024mamba}; masked or non-autoregressive infill models such as BERT~\citep{devlin2019bert} and MaskGIT~\citep{chang2022maskgit}; and benchmark families that stress multi-hop reasoning, code completion, and protein sequence modeling~\citep{yang2018hotpotqa,clark2018arc,rajpurkar2016squad,austin2021program,rives2021biological}. Our contribution is not a new benchmark or a new backbone, but a controlled study of one lightweight cross-layer mechanism across both synthetic repair tasks and a realistic post-training search archive.

\section{Conclusion}
Future-Seed adds a small amount of cross-layer state feedback to a recurrent stack, but its effect is highly structured. The synthetic evidence is strong: future-aware repair, permutation repair, and Sudoku all improve materially. The real-task evidence is narrower but still useful: \texttt{protein\_ss} is a robust positive family, while \texttt{hotpot}, \texttt{punc}, and \texttt{squad} provide supporting gains and \texttt{mbpp\_longctx}/\texttt{arc\_mc} remain mixed. The most defensible conclusion is therefore targeted rather than universal: Future-Seed is a practical mechanism for in-place constraint repair, and a promising but still unstable ingredient for broader post-training.
